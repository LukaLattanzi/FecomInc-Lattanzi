...
-- -------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE sp_listar_productos_ordenados (
    IN campo_orden VARCHAR(255),     -- Campo por el cual se desea ordenar
    IN tipo_orden VARCHAR(10)        -- 'ASC' para ascendente o 'DESC' para descendente
)
BEGIN
    -- Procedimiento que devuelve todos los productos ordenados dinámicamente
    SET @query = CONCAT(
        'SELECT Product_ID, Product_Category_Name, Product_Weight_Gr, ',
        'Product_Length_Cm, Product_Height_Cm, Product_Width_Cm ',
        'FROM products ORDER BY ',
        campo_orden, ' ', tipo_orden
    );
...
-- -------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE sp_actualizar_peso_producto (
    IN producto_id VARCHAR(255),
    IN nuevo_peso DECIMAL(12, 2)
)
BEGIN
    -- Este procedimiento actualiza el peso de un producto,
    ...
    -- Validamos existencia y valor del peso
    IF existe = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: El producto no existe.';
    ELSEIF nuevo_peso <= 0 THEN
        SIGNAL SQLSTATE '45000'
