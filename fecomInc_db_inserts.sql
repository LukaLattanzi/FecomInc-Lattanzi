...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_geolocations.csv'
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_products.csv'
INTO TABLE products
CHARACTER
SET utf8mb4
FIELDS
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_sellers.csv'
INTO TABLE sellers
CHARACTER
SET utf8mb4
FIELDS
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_customers.csv'
...
SET 
    Subscribe_Date = IF(@sub_date = '', NULL, STR_TO_DATE(@sub_date, '%Y-%m-%d')),
    First_Order_Date = IF(@first_order = '', NULL, STR_TO_DATE(@first_order, '%Y-%m-%d'));
-- -------------------------------------------------------
DELETE FROM temp_customers 
WHERE Customer_Trx_ID = '' OR Customer_Trx_ID IS NULL;
-- -------------------------------------------------------
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_orders.csv'
...
SET
  Order_Purchase_Timestamp = IF(@purchase_ts = '', NULL, 
                               STR_TO_DATE(REPLACE(REPLACE(@purchase_ts, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
  Order_Approved_At = IF(@approved_at = '', NULL, 
                         STR_TO_DATE(REPLACE(REPLACE(@approved_at, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
  Order_Delivered_Carrier_Date = IF(@deliv_carrier = '', NULL, 
                                   STR_TO_DATE(REPLACE(REPLACE(@deliv_carrier, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
  Order_Delivered_Customer_Date = IF(@deliv_cust = '', NULL, 
                                    STR_TO_DATE(REPLACE(REPLACE(@deliv_cust, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
  Order_Estimated_Delivery_Date = IF(@est_deliv = '', NULL, 
                                    STR_TO_DATE(REPLACE(REPLACE(@est_deliv, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s'));
-- -------------------------------------------------------
DELETE t FROM temp_orders t
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_ordersReviews.csv'
...
SET
  Review_Creation_Date = IF(@rev_date = '', NULL, 
                          STR_TO_DATE(REPLACE(REPLACE(@rev_date, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
  Review_Answer_Timestamp = IF(@ans_ts = '', NULL, 
                             STR_TO_DATE(REPLACE(REPLACE(@ans_ts, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s'));
-- -------------------------------------------------------
DELETE tor FROM temp_order_reviews tor
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_ordersPayments.csv'
...
SET
    Payment_Value = CAST(@payment_value AS DECIMAL(12, 2));
-- -------------------------------------------------------
DELETE top FROM temp_order_payments top
LEFT JOIN orders o ON top.Order_ID = o.Order_ID
WHERE o.Order_ID IS NULL;
-- -------------------------------------------------------
INSERT INTO order_payments
SELECT * FROM temp_order_payments;
-- -------------------------------------------------------
...
-- -------------------------------------------------------
LOAD DATA INFILE '/var/lib/mysql-files/fecomInc_ordersItems.csv'
...
SET
    Shipping_Limit_Date = IF(@shipping_limit_date = '', NULL, 
                           STR_TO_DATE(REPLACE(REPLACE(@shipping_limit_date, 'T', ' '), '.000Z', ''), '%Y-%m-%d %H:%i:%s')),
    Price = IF(@price = '', NULL, CAST(@price AS DECIMAL(12, 2))),
    Freight_Value = IF(@freight_value = '', NULL, CAST(@freight_value AS DECIMAL(12, 2)));
-- -------------------------------------------------------
DELETE toi FROM temp_order_items toi
